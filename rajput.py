...
TELEGRAM_BOT_TOKEN = '7666474705:AAGfZ_I255GOfARtcenYwh4setUg6aUdSO8'
...
async def start(update: Update, context: CallbackContext):
    ...
    if not await is_user_allowed(user_id):
        await context.bot.send_message(chat_id=chat_id, text="*❌ YE BOT FREE NAHI H BUY KARO @RAJPUTDDOS SE ❌! /get_id*", parse_mode='Markdown')
        return
    message = (
...
async def attack(update: Update, context: CallbackContext):
    ...
    user_id = update.effective_user.id
    if not await is_user_allowed(user_id):
        await context.bot.send_message(chat_id=chat_id, text="*❌ YE BOT FREE NAHI H BUY KARO @RAJPUTDDOS SE ❌!*", parse_mode='Markdown')
        return
    args = context.args
    if len(args) != 3:
